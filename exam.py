...
def fetch_tweets(since_date, target_count):
    url = "https://api.twitterapi.io/twitter/tweet/advanced_search"
    since_str = datetime.combine(since_date, datetime.min.time()).strftime("%Y-%m-%d_%H:%M:%S_UTC")
    until_str = datetime.utcnow().strftime("%Y-%m-%d_%H:%M:%S_UTC")

    query = (
        "((\"pump and coin\" OR memecoin OR \"pump and fun\" OR Solana) "
        "min_retweets:20 min_replies:10 lang:en -filter:retweets)"
    )
    params = {
        "query": query,
        "queryType": "Latest"
    ...
    while len(all_tweets) < target_count:
        ...
        if response.status_code == 200:
            ...
            filtered_tweets = [
                ...
                if tweet.get("retweet_count", 0) > 20
                and tweet.get("reply_count", 0) > 10
                and tweet.get("user", {}).get("screen_name", "") not in ["", "unknown"]
            ]
