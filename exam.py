    ...
    def fetch_paginated(query_string, is_official=False, max_tweets=limit):
        ...
        while tweets_fetched < max_tweets and max_attempts > 0:
            try:
                ...
                st.write(f"DEBUG: Выполняется запрос: {params['query']}, next_cursor={next_cursor}, попытка={6-max_attempts}")
                response = session.get(url, headers=headers, params=params, timeout=10)
                response.raise_for_status()
                data = response.json()
                ...
                max_attempts -= 1
                if max_attempts == 0:
                    st.write(f"DEBUG: Достигнуто максимальное количество попыток ({max_attempts}) для запроса '{query_string}'")
                    break
