...
from pinecone import Pinecone, ServerlessSpec
from langgraph.prebuilt import create_react_agent
from langchain.tools import Tool  # Импорт инструмента для агента
...
# Функция-инструмент для поиска документов
def doc_search_tool(query: str) -> str:
    docs = vector_store.similarity_search(query, k=3)
    if not docs:
        return "Документи не знайдені."
...
search_tool = Tool(
    ...
    description="Інструмент для пошуку релевантних документів у векторній базі. Приймає текст запиту і повертає тексти документів."
)
...
if uploaded_file is not None:
    ...
    # Обновление JSON не обязательно для поиска, можно раскомментировать если нужно сохранять
    # with open(json_path, "w", encoding="utf-8") as jf:
